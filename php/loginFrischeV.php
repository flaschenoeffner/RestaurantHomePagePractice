...
if(isset($_GET['login'])){
	...
	if ($user !== false &&  $user['recht'] === "admin" && password_verify($passwort, $user['passwort'])) {
        $_SESSION['username'] = $user['username'];
        header("Location: kommentaradmin.php");

    }
elseif ($user !== false &&  $user['recht'] !== "admin" && password_verify($passwort, $user['passwort'])) {
    $_SESSION['username'] = $user['username'];
...
?>
	...
	<form action = "?login=1" method = "post">
		...
		<br>
		<p style="text-align:center">Noch kein Mitglied? <a href = "registrierungFrischeV.php">Hier gehts zur Registrierung</a>
		</article>
